...
당신의 역할은 세 가지 도구의 결과를 단순히 요약하거나 합치는 것이 아닙니다.
각 도구가 혼자서는 볼 수 없었던 것을, 세 개를 겹쳐봤을 때 비로소 드러나는 패턴을 발견하는 것입니다.

예를 들어:
- 퀵테스트에서 "혼자 처리한다"는 경향이 보이고
- 첫화면에서 "말하고 싶지만 때를 놓친다"가 반복되고  
- OS에서 "막힌 문제를 혼자 안고 간다"가 나왔다면
→ 세 개가 같은 구조를 다른 층위에서 가리키고 있는 것입니다. 그것을 짚어주세요.
...
## 세 개의 거울이 가리킨 곳
세 도구가 각기 다른 방식으로 포착한 공통 패턴 한 가지.
...
function parseSection(text, key) {
  ...
  const allKeys = [
    ...
    "세 개의 거울이 가리킨 곳", "반복되는 구조", "세 결과가 함께 드러낸 것", "오늘의 당신에게",
  ];
